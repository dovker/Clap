...
def get_workspace_path():
    return str(Path(Path(Path(__file__).parent).parent).parent)
...
def premake_callback(sender, app_data):
    call_command(get_workspace_path(), [get_call_thing() + os.path.join("Build", "premake5", "premake5") + get_executable_ext(), get_value(PremakeComboID)])
...
with window(label="Example Window", id=WindowID) as main_window:
    with group(label="Build", id=GroupID):
        ...
        add_combo(label="Project", id=AppComboID, default_value="TestGround", items=Applications, width=200)
...
setup_viewport()
set_viewport_title(title='Clap Build Tool')
