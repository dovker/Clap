...
namespace Clap
{
	...
	{
    public:
        bgfxTexture2D(const std::string& path);
        bgfxTexture2D(uint32_t width, uint32_t height, TextureSpecification specification);
        bgfxTexture2D(const Ref<ByteBuffer>& buffer);
        ~bgfxTexture2D();
        ...
        {
            return m_ID == ((bgfxTexture2D&)other).m_ID;
        }
